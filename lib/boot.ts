...
(function(){var B=document.getElementById('boot');if(!B)return;var S=B.querySelector('.boot-search'),V=B.querySelector('.boot-list'),W=window,I=B.querySelector('input'),L=B.querySelector('ul'),R=[],t,c=0,
...
function area(lat,lon,name,cc){if(W.__hyd&&!W.__bootAreaOk)return;var KH={hotel:'מלון',guest_house:'גסטהאוס',hostel:'הוסטל',alpine_hut:'טי-האוס / לודג׳',motel:'מוטל',lodge:'לודג׳'};W.__bootArea={lat:lat,lon:lon,p:fetch('/api/area?lat='+lat+'&lon='+lon).then(function(r){return r.ok?r.json():null}).catch(function(){return null})};
var sk='';for(var k=0;k<4;k++)sk+='<li class="card skeleton row-skel"></li>';
V.innerHTML='<header class="header scrolled"><span class="brand"><img src="/icons/v2/icon-96.webp" alt="" width="32" height="32"><span>מחיר ללילה</span></span></header><main class="wrap"><div class="area-head"><div><p class="muted small">מקומות לינה</p><h1 class="area-title">'+(cc?'<span class="flag">'+fl(cc)+'</span>':'')+'<bdi>'+es(name)+'</bdi></h1></div></div><div class="map map-loading">טוען מפה…</div><ul class="places">'+sk+'</ul></main>';V.hidden=false;
W.__bootArea.p.then(function(j){if(!j||!V.isConnected)return;var c=j.counts||{},ps=(j.places||[]).map(function(p){return{id:p.id,name:p.name,kind:p.kind,d:km(lat,lon,p.lat,p.lon)}});
(j.reported||[]).forEach(function(r){if(!ps.some(function(p){return p.id==r.id}))ps.push({id:r.id,name:r.name,kind:r.kind||'guest_house',d:km(lat,lon,r.lat,r.lon)})});ps.sort(function(a,b){return a.d-b.d});
if(!ps.length)return;V.querySelector('.places').innerHTML=ps.map(function(p){var n=c[p.id]||0;return'<li><div class="card place"><div class="thumb sm ph ph-'+es(p.kind)+'" aria-hidden="true"><span>'+(ICON[p.kind]||'🏠')+'</span></div><span class="place-body"><span class="name" dir="auto">'+es(p.name)+'</span><span class="muted small">'+(KH[p.kind]||'לינה')+' · '+dst(p.d)+'</span></span>'+(n?'<span class="badge price-wait" aria-label="טוען מחיר"><i></i></span>':'<span class="badge empty">עוד אין מחיר</span>')+'</div></li>'}).join('')})}
...
function pick(s){if(!s)return;W.__boot.pick=s;I.blur();L.hidden=true;B.querySelector('.boot-wait').hidden=false;if(s.type!='stay'&&!W.__bootPick){area(s.lat,s.lon,s.name,s.country)}if(W.__bootPick)W.__bootPick(s)}

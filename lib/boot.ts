...
if(!ps.length)return;var U=V.querySelector('.places'),k=ps.filter(function(p){return c[p.id]}).length;U.insertAdjacentHTML('beforebegin','<div class="seg filter"><button type="button" class="on">הכל ('+ps.length+')</button><button type="button">עם מחירים ('+k+')</button></div>');U.innerHTML=ps.map(function(p){var n=c[p.id]||0;return'<li><div class="card place"><div class="thumb sm ph ph-'+es(p.kind)+'" aria-hidden="true"><span>'+(ICON[p.kind]||'🏠')+'</span></div><span class="place-body"><span class="name" dir="auto">'+es(p.name)+'</span><span class="muted small">'+(KH[p.kind]||'לינה')+' · '+dst(p.d)+'</span></span>'+(n?'<span class="badge price-wait" aria-label="טוען מחיר"><i></i></span>':'<span class="badge empty">עוד אין מחיר</span>')+'</div></li>'}).join('')})}
